/* ============================================================
   NexHub — app.js  (v5 — enhanced animations & UX)
   ============================================================ */
...
function toggleFavorite(url, e) {
  ...
  // Animate the card-wrap without full re-render
...
async function init() {
  try {
    ...
    setupSearch();
    setupScrollArrows();
    animateCount(0, allSites.length);
  } catch (err) {
...
// Animate the site count number rolling up
function animateCount(from, to) {
  ...
  function tick(now) {
    ...
    el.textContent = `${val} sites`;
    if (t < 1) requestAnimationFrame(tick);
    else {
      el.textContent = `${to} site${to !== 1 ? 's' : ''}`;
...
function buildCategoryNav() {
  ...
  favBtn.addEventListener('click', () => {
    activeCategory = '__favorites__'; searchQuery = ''; visibleCount = PAGE_SIZE;
    document.getElementById('searchInput').value = '';
    setActiveBtn(favBtn); render();
  });
  nav.querySelector('[data-cat="all"]').insertAdjacentElement('afterend', favBtn);
  ...
  allCategories.forEach(cat => {
    ...
    btn.addEventListener('click', () => {
      activeCategory = cat.id; searchQuery = ''; visibleCount = PAGE_SIZE;
      document.getElementById('searchInput').value = '';
      setActiveBtn(btn); render();
    });
    nav.appendChild(btn);
  });
}
...
function setupSearch() {
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape' && document.activeElement === input) {
      ...
      searchQuery = ''; visibleCount = PAGE_SIZE; render();
    }
  });
}

document.querySelector('.cat-btn[data-cat="all"]').addEventListener('click', function () {
  activeCategory = 'all'; searchQuery = ''; visibleCount = PAGE_SIZE;
  document.getElementById('searchInput').value = '';
  setActiveBtn(this); render();
});

// ── Filter logic ─────────────────────────────────────────────
function getFiltered() {
  if (activeCategory === '__favorites__') {
    return allSites.filter(s => favorites.has(s.url));
  }
...
function render() {
  ...
  if (showPinned) {
    ...
    if (visibleCount < rest.length) renderLoadMore(rest.length, rest.length - visibleCount);
  } else {
    grid.innerHTML = filtered.slice(0, visibleCount).map(s => renderCard(s)).join('');
    if (visibleCount < filtered.length) renderLoadMore(filtered.length, filtered.length - visibleCount);
  }
}
...
function renderCard(site) {
  ...
  return `<div class="card-wrap${isFav ? ' card-wrap--fav' : ''}">
    <a class="card" href="${escHtml(site.url)}" target="_blank" rel="noopener noreferrer" style="--card-color:${color}">
      <div class="card-top">
        ${iconHtml}
    ...
    <button class="${favClass}" onclick="toggleFavorite('${escHtml(site.url)}', event)">${escHtml(favLabel)}</button>
  </div>`;
}
